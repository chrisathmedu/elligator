...
-- TODO below are just here s.t. they are directly useable in new vars *Propertery files
-- i.e. 3A only depends on previous stuff
-- => move all into A B C files?
